...
typedef struct ms_dfinity_code_run_t {
	sgx_status_t ms_retval;
} ms_dfinity_code_run_t;
...
static sgx_status_t SGX_CDECL sgx_dfinity_code_run(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_dfinity_code_run_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_dfinity_code_run_t* ms = SGX_CAST(ms_dfinity_code_run_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = dfinity_code_run();
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_dfinity_code_run, 0},
