...
void MTGetMerkleProof(MerkleTree* treeptr, int nodeidx, MerkleProof* pointer)	
{
	int arridx = 0;
...
void MTBuildTreeFromDB(MerkleTree* treeptr, const char* filename)
{
	...
	for (int i = 0; i < totalNumDB; ++i)
	{
		...
		strncpy(temp, databuf[i], strlen(databuf[i]));

		char* remaining;
		treeptr->nodes[arridx].hash = strtoul(temp, &remaining, 10); // return a pointer of the remaining part to `value`
...
int test()
{
	...
	MerkleProof* mpptr = malloc(sizeof(MerkleProof)); 
	MTGetMerkleProof(tree, MTLeafIdxToTreeIdx(4), mpptr);
	printf("%s\n", "\n\n\n");
